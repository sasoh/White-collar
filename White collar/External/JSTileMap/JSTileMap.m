...
+ (JSTileMap*)mapNamed:(NSString*)mapName withBaseZPosition:(CGFloat)baseZPosition andZOrderModifier:(CGFloat)zOrderModifier
{
...
#ifdef DEBUG
			DDLogWarn(@"Layer %@ has zPosition %f", layerInfo.name, child.zPosition);
#endif
			[map addChild:child];
		}
	}
	...
	{
#ifdef DEBUG
		DDLogWarn(@"Object Group %@ has zPosition %f", objectGroup.groupName, (baseZPosition + (map.zOrderCount - objectGroup.zOrderCount) * zOrderModifier));
#endif
...
//#warning This needs to be optimized into tilemap layers like our regular layers above for performance reasons.
	...
	for (TMXImageLayer* imageLayer in map.imageLayers)
	{
		...
		[map addChild:image];
#ifdef DEBUG
		DDLogWarn(@"IMAGE Layer %@ has zPosition %f", imageLayer.name, image.zPosition);
#endif
...
-(void)parser:(NSXMLParser *)parser didStartElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName attributes:(NSDictionary *)attributeDict
{
	if([elementName isEqualToString:@"map"])
	{
		...
		{
			DDLogWarn(@"Unsupported orientation: %@", attributeDict[@"orientation"]);
			[parser abortParsing];
		}
	...
	else if([elementName isEqualToString:@"tileset"])
	{
		...
		{
			DDLogWarn(@"External tilesets unsupported!");
			[parser abortParsing];
			return;
		}
	...
	else if([elementName isEqualToString:@"property"])
	{
		...
		{
			DDLogWarn( @"TMX tile map: Parent element is unsupported. Cannot add property named '%@' with value '%@'", attributeDict[@"name"], attributeDict[@"value"]);
		}
...
- (void)parser:(NSXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName
{
	...
	if([elementName isEqualToString:@"data"])
	{
		...
		if (layerAttributes & TMXLayerAttributeBase64)
		{
			...
			if( ! buffer.length ) {
				DDLogWarn(@"TiledMap: decode data error");
				[parser abortParsing];
				return;
			}
			...
			if( layerAttributes & (TMXLayerAttributeGzip | TMXLayerAttributeZlib) )
			{
				...
				if( ! deflated )
				{
					DDLogWarn(@"TiledMap: inflate data error");
					[parser abortParsing];
					return;
				}
...
{
	DDLogWarn(@"Error on XML Parse: %@", [parseError localizedDescription]);
}
